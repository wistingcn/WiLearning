...
const logger = log4js_1.getLogger('Room');
let mediaCodecs = new Array();
(_a = mediasoup_1.getSupportedRtpCapabilities().codecs) === null || _a === void 0 ? void 0 : _a.forEach(codec => {
...
class Room extends events_1.EventEmitter {
    constructor(id, mediasoupRouter) {
        super();
        this.id = id;
        ...
        this.classRoom = {
            startTime: 0,
            stopTime: 0,
            status: defines_1.RoomStatus.stopped,
            logoUrl: '',
            announcementText: '',
            videoFilter: false,
        };
    ...
    setupSocketHandler(peer) {
        peer.socket.on('request', (request, cb) => {
            this.setActive();
            logger.debug('Peer "request" event [room:"%s", method:"%s", peerId:"%s"]', this.id, request.method, peer.id);
    ...
    checkDeserted() {
        ...
        const lastActive = (Date.now() - this.activeTime) / 1000;
        if (lastActive > 2 * 60 * 60) {
    ...
    stopClass(peer) {
        ...
        this.classRoom.status = defines_1.RoomStatus.stopped;
    ...
    async _handleSocketRequest(peer, request, cb) {
        switch (request.method) {
            ...
            case 'join':
                {
                    const { roler, displayName, picture, platform, rtpCapabilities } = request.data;
                    if (peer.joined) {
                        cb(null, { joined: true });
                        break;
                    }
                    ...
                    logger.debug('peer joined [peer: "%s", displayName: "%s", picture: "%s", roler:"%s", platform: "%s"]', peer.id, displayName, picture, roler, platform);
                    peer.joined = true;
                    break;
                }
            case 'createWebRtcTransport':
                {
                    ...
                    const { maxIncomingBitrate, initialAvailableOutgoingBitrate } = config_1.lConfig.webRtcTransport;
                    const transport = await this.mediasoupRouter.createWebRtcTransport({
                        listenIps: config_1.lConfig.webRtcTransport.listenIps,
                        enableUdp: !forceTcp,
                        enableTcp: true,
                        preferUdp: true,
            ...
            case 'classStart':
                {
                    ...
                    this.classRoom.status = defines_1.RoomStatus.started;
